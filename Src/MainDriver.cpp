...
int main()
{
	...
	cout << "Step (the number of iteration) for Hill climbing : <input type = \"text\" name = \"step\"><br><br>" << endl << endl;
	...
	cout << "Maximum Iteration for Simulated Annealing (default : 2600) : <input type = \"text\" name = \"maxIter\"><br>" << endl;
	...
	cout<<"</form>"<<endl;
 	...
 	if(Get.find("algo")==Get.end() || Get.find("userfile")==Get.end()){
		...
		if(Get["algo"] == "HillClimbing" && Get["step"] == ""){
			cout << "<p style=\"font-family: Arial;\"><b>Fill up the number of iteration if you select Hill Climbing<b></p>" <<endl;
		} else if(Get["algo"] == "SimulatedAnnealing" && (Get["temperature"] == "" || Get["descentRate"] == "" || Get["maintainAttempts"] == "" || Get["maxIter"] == "")) {
			cout << "<p style=\"font-family: Arial;\"><b>Fill up the text boxes for Simulated Annealing<b></p>" << endl;
		} else {
